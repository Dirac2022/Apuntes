...
**Buckets** son contenedores de almacenamiento utilizados en servicios de **almacenamiento de objetos en la nube**, como **Amazon S3 (Simple Storage Service)**.

Un **bucket** es una unidad lógica que sirve para guardar archivos (_objetos_) en plataformas de almacenamiento en la nube. Piensa en un bucket como una carpeta gigante donde puedes subir fotos, documentos, archivos de configuración, backups, etc.

**📦 Ejemplo práctico:**

Imagina que tienes una aplicación web que necesita servir imágenes a los usuarios. Puedes subir todas las imágenes a un bucket llamado `imagenes-app` en Amazon S3.

```bash
aws s3 mb s3://imagenes-app  # Crear un bucket
aws s3 cp foto1.jpg s3://imagenes-app/  # Subir una imagen
```

También puedes usar buckets para guardar:

- Logs del sistema
- Backups de bases de datos
- Archivos estáticos de sitios web
...
**IAM** significa **Identity and Access Management** (Identidad y Gestión de Acceso). Es un sistema que permite controlar **quién puede hacer qué** en tu infraestructura.

Una **política de IAM** es un documento (normalmente en formato JSON) que define permisos. Estas políticas **autorizan o restringen** acciones sobre los recursos en la nube (como acceder a un bucket, lanzar una máquina virtual, leer una base de datos, etc.).
  ...
  "Statement": [
    {
      "Effect": "Allow",
      "Action": ["s3:GetObject"],
      "Resource": ["arn:aws:s3:::imagenes-app/*"]
    }
  ]
}
```
...
- Permite (Allow)
- La acción `s3:GetObject` (leer archivos del bucket)
- En el recurso `imagenes-app` (todos los archivos `*` del bucket)
    
---

### ✅ Conexión con IaC

Cuando usas herramientas como **Terraform**, puedes definir **buckets** y sus **políticas de IAM** en archivos de configuración en lugar de hacerlo manualmente por consola:
...
resource "aws_iam_policy" "leer_imagenes" {
  name   = "LeerImagenes"
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [{
...
Excelente, veamos primero **qué es Nginx**, y luego paso a explicarte **el playbook de Ansible línea por línea** para que entiendas cómo se relaciona con el principio de **idempotencia** en Infraestructura como Código.

---

# 🚀 nginx

**Nginx** (pronunciado "engine-x") es un **servidor web** y **proxy inverso** muy popular.

**Funciones principales:**

- Servir contenido web (HTML, CSS, JS)
- Actuar como **balanceador de carga**
- Funcionar como **proxy inverso** para redirigir peticiones a aplicaciones backend (por ejemplo, apps en Node.js, Django, etc.)
- Servir como **caché de contenido** para mejorar el rendimiento

> [!tip] Es una alternativa ligera y de alto rendimiento a Apache, muy usada en producción.
