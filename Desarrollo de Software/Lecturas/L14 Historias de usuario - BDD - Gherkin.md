...
Esto se traduce en un ciclo de retroalimentación ágil, donde cada iteración genera una base de pruebas que se ejecuta de forma recurrente, garantizando que el software sigue alineado con las expectativas del usuario.
...
Supón que estás construyendo un sistema para una app bancaria que tiene esta funcionalidad:

> **Transferencia de dinero entre dos cuentas bancarias.**

El proceso completo:

- El usuario inicia una transferencia de 100 € desde su cuenta.
...
Esto involucra varias piezas de código: `Cuenta`, `ServicioTransferencia`, `RepositorioTransacciones`, `NotificadorEmail`.

---

###### 🔬 1. **Prueba unitaria**

Aquí probamos **una sola función/método aislado**, sin dependencias externas.
...
✅ Verificamos que el método `retirar()` funcione correctamente en **aislamiento**, sin llamadas a base de datos, sin notificaciones, sin lógica de negocio externa.

---

###### 🔗 2. **Prueba de integración**

Aquí probamos que **varias unidades colaboran correctamente entre sí**, por ejemplo: que `ServicioTransferencia` use bien a `Cuenta`, `Repositorio` y `Notificador`.
...
✅ Aquí **no probamos el sistema entero**, pero sí la **colaboración entre objetos**. Comprobamos que las piezas se “hablan” bien entre ellas.  
✳️ **Diferencia con prueba del sistema:** aún estamos controlando el entorno (usamos mocks, no una app real).
...
def test_transferencia_via_api():
    # Simulación real: se usa una API y DB reales (o staging)
...
✅ Ahora probamos todo el stack: API → servicios → base de datos → lógica de negocio → notificación.  
🆚 **Diferencia clave con integración:** ya **no usamos mocks ni controlamos manualmente las dependencias**.

---

###### ✅ 4. **Prueba de aceptación (BDD)**

Aquí nos ponemos en los zapatos del usuario y verificamos si **la funcionalidad cumple con la historia de usuario escrita en lenguaje natural**.
...
Scenario: Transferencia exitosa entre cuentas
  ...
  And debo recibir una notificación de éxito
```
...
✅ Estas pruebas suelen ser **automatizadas con Behave** o herramientas similares y reflejan el comportamiento esperado **desde el punto de vista del usuario final**.

---

###### 🧠 Diferencia clave: **Integración vs Sistema**

|Aspecto|Prueba de integración|Prueba del sistema|
|---|---|---|
|¿Qué prueba?|La colaboración entre varios componentes|El sistema completo como un todo|
|¿Usa mocks o stubs?|Sí, normalmente|No, usa servicios reales|
|¿Involucra base de datos o red?|No necesariamente|Sí, lo más común|
|¿Simula un entorno real?|No del todo|Sí (API, BD, red, archivos, etc.)|
|¿Inicia desde una API/UI?|No|Sí (por ejemplo, un `client.post()` o Selenium)|
