...
El archivo `main.tf.json` generado por el script `generate_infra.py` crea 15 `null_resources`. Los 14 primeros sirven como `placeholders` donde se definen atributos como 

- `factory_uuid`
- `timestamp`
- `index`

Estos atributos están condicionados por un `trigger`, lo que significa que forzaran ejecución del `provisioner` nuevamente, si es que los valores de estos atributos han cambiado. Por lo que fuerzan la creación del recurso.

Pude observar que todos los `placeholders` comparten el mismo identificador `8d4f49fa-e81a-4462-8934-8c5b8e7fc41e`. Esto me indica que los `placeholders` generados están asociados a un mismo proceso de creación. Esto es fácil de ver ya que todos los recursos han sido creados mediante el mismo script `generate_infra.py`. Es por ello que también comparten el mimo `timestamp`.

La forma de distinguir cada `placeholder` es mediante un atributo `index` que sirve como identificador.


Al final del bloque `resource` hay un ultimo `null_resource` de tipo `finalizador`. Este recurso tiene un `factory_uuid` y un `timestamp` distinto: `20303c8-1179-4f18-b3cd-d6529f121b27`. Esto se debe a que tiene un rol distinto.
