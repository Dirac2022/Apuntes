...
- Procedí a crear un entorno virtual
- Corregí la ruta para Python

```hcl
variable "python_executable" {
description = "Ruta al ejecutable de Python (python o python3)."
type = string
default = "/home/dirac/Documents/DS/Actividad19/Proyecto_iac_local/.venv/bin/python"
}
```


- No tuve inconvenientes al ejecutar `terraform init`  y `terraform plan`
- **Tuve inconvenientes** al ejecutar `terraform apply`

```sh
│ Error: local-exec provisioner error
│ 
│   with module.simulated_apps["app1"].null_resource.start_service_sh,
│   on modules/application_service/main.tf line 70, in resource "null_resource" "start_service_sh":
│   70:   provisioner "local-exec" {
│ 
│ Error running command './scripts/bash/start_simulated_service.sh 'app1'
│ '/home/dirac/Documents/DS/Actividad19/Proyecto_iac_local/generated_environment/services/app1_v1.0.2'
│ '/home/dirac/Documents/DS/Actividad19/Proyecto_iac_local/generated_environment/services/app1_v1.0.2/config.json'': exit status 126.
│ Output: bash: line 1: ./scripts/bash/start_simulated_service.sh: Permission denied
│ 
╵
╷
│ Error: local-exec provisioner error
│ 
│   with module.simulated_apps["app2"].null_resource.start_service_sh,
│   on modules/application_service/main.tf line 70, in resource "null_resource" "start_service_sh":
│   70:   provisioner "local-exec" {
│ 
│ Error running command './scripts/bash/start_simulated_service.sh 'app2'
│ '/home/dirac/Documents/DS/Actividad19/Proyecto_iac_local/generated_environment/services/app2_v0.5.0'
│ '/home/dirac/Documents/DS/Actividad19/Proyecto_iac_local/generated_environment/services/app2_v0.5.0/config.json'': exit status 126.
│ Output: bash: line 1: ./scripts/bash/start_simulated_service.sh: Permission denied
```

- Lo solucioné simplemente aplicando permiso de ejecución al archivo `start_simulated_service.sh`

```
chmod +x start_simulated_service.sh 
```

Luego de aplicar `terraform apply` nuevamente se crearon los archivos y carpetas:
- `terraform.lock.hcl`
- `.terraform/`
- `generated_environment/`junto con los archivos y subdirectorios correspondientes
- `terraform.tfstate.backup`

<img src="img19/Pasted image 20250527102117.png" >


Y se modificaron los archivos :
- `start_simulated_service.sh`
- `terraform.tfstate` 

<img src="img19/act19-1.png" >
...
de `main.tf` en `modules/environment_setup` y lo coloqué en `variables.tf` 

>[!note] Observación
>Al usar el IDE vscode y tener instalado la extensión de terraform pude notar que al eliminar el bloque que define las variables en el archivo `main.tf` automáticamente se generaron muchos errores, esto se debe obviamente a que las variables no están definidas, este error se soluciono inmediatamente después de colocar la definición de las variables en el archivo `variables.tf`

<img src="img19/act19-2.png" >
...
Se modificó el archivo `main.tf` para usar el módulo `environment_setup`. Con ello:

- Se definió la ruta del módulo en `source`
	
- Se asigno el valor de la variable  `nombre_entorno` (la cual se definió en el archivo `variables.tf` principal) a la variable `nombre_entorno_modulo` (definida en el archivo `variables.tf` del módulo `environment_setup`)
- Se definió un `output` `readme_principal` cuyo valor lo toma de un `output` definido en el `main.tf` del módulo `environment_setup`:

<img src="img19/act19-3.png" >


> [!warning] Tuve problemas al aplicar `terraform apply` después del cambio.

Para resolverlo comente el `working_dir`en `main.tf` de `environment_setup`
...
resource "null_resource" "ejecutar_setup_inicial" {
  ...
  provisioner "local-exec" {
    command     = "bash ${path.module}/scripts/initial_setup.s '${var.nombre_entorno_modulo}' '${local_file.readme_entorno.filename}'"
    interpreter = ["bash", "-c"]
    //working_dir = "${var.base_path}/${var.nombre_entorno_modulo}_data" # Ejecutar script desde aquí
  }
}
...
Otras opción que hallé fue:

- Cambiar: `${path.module}/scripts/initial_setup.sh`
- Por `${path.cwd}/modules/environment_setup/scripts/initial_setup.sh


Agregué este bloque en `main.tf` de `environment_setup` para tener una idea de las rutas:

```hcl
output "ruta_path_module_pc" {
  value = "${path.cwd}/modules/environment_setup/scripts/initial_setup.sh"
}

output "ruta_path_module_pm" {
  value = "${path.module}/scripts/initial_setup.sh"
}

output "ruta_working_dir" {
  value = "${var.base_path}/${var.nombre_entorno_modulo}_data"
}

```

<img src="img19/act19-4.png" >
...
>[!note] Desarrollo
>Simplemente copie el bloque que definía las variables `app_name`, `app_version`, `app_port` , `base_install_path` , `global_message_from_root` , `python_exe` del archivo `main.tf` en `application service` hacia el archivo `variables.tf` de `application_service`
>
> Al aplicar `terraform apply` no obtuve errores
> 
> <img src="img19/act19-5.png" >
...
Solo agregué estos cambios en `generate_app_metadata.py` para reducir la verbosidad de la salida

<img src="img19/act19-6.png" >

>[!important] No tuve errores al ejecutar `terraform apply`
...
Añadí el nuevo servicio como  `dbconector` con sus respectivos atributos.
```go
locals {
  common_app_config = {
    // ...
    dbconector = { 
      version = "1.5.2", 
      port = 5432
      custom_connection_string = "postgresql://user:pass@localhost:5432/mydb_simulated"
    }
  }
}
```

Tambien actualicé el modulo `simulated_apps`
...
Añadí la nueva variable en `variables.tf` de `application_service`
...
Actualicé el archivo con el campo `connectionString`
...
>[!warning] Falta completar esta parte. No usé condicionales
...
def perform_complex_validations(config_data, file_path):
    ...
    if app_name == "database_connector":
        ...
        if not connection_string:
            errors.append(f"[{file_path}] Para 'connection_string' vacio para 'database_connector")
        if not isinstance(connection_string, str):
            errors.append(f"[{file_path}] Para 'database_connector', 'connectionString' ('{connection_string}') debe ser de tipo string, pero es de tipo {type(connection_string)}")

    return errors, warnings
```

> [!note] Las únicas validaciones que impuse fueron que la variable sea no nula y de tipo string
...
if [ "$APP_NAME" == "database_connector"]; then
    ...
    touch "$DB_LOCK_FILE"
    if [$? -eq 0]; then
      echo "Grupo 1 Desarrollo ha realizado el reto adicional Ej-1: crear un archivo .db_locl"
      echo "Se ha creado el archivo .db_lock en la ruta: $INSTALL_PATH" >> "$LOG_FILE"
    else
      echo "ERROR al crear .db_locl para $APP_NAME." >> $LOG_FILE
    fi
# Crear un archivo PID simulado
# ...
```

> [!tip] Se ejecutó `terraform apply` satisfactoriamente


<img src="img19/Pasted image 20250601004153.png" >
...
def main():
    deployment_id = f"dep-{str(uuid.uuid4())}"

    global_metadata = {
        "deployment_id": deployment_id
    }

    print(json.dumps(global_metadata))
...
También añado un nuevo parámetro en `simulated_apps`
...
module "simulated_apps" {
  ...
  deployment_id_global = local.global_deployment_id
}
```
...
Agregué la nueva variable en `variables.tf` del módulo  `application_service`

```go
// ...
variable "deployment_id_global" {
    type = string
    description = "ID de despliegue global"
}
```
...
Primero agregue el parámetro en `data "external" "app_metadata_py` 
  ...
  query = merge(
    {
      app_name   = var.app_name
      version    = var.app_version
      global_deployment_id = var.deployment_id_global // Aqui
      input_data = "datos_adicionales_para_python"
    },
    {
      q1  = "v1",  q2  = "v2",  q3  = "v3",  q4  = "v4",  q5  = "v5",
...
Posteriormente, modifiqué el `generate_app_metadata.py`

```python
def main():
    # ...
    global_deployment_id = input_json.get("global_deployment_id", "GLOBAL_ID_NOT_PROVIDED")
    # Lógica de generación de metadatos 
    metadata = {
        "appName": app_name,
        "appVersion": app_version,
        "globalDeploymentId": global_deployment_id,
        # ...
    }
```
...
<img src="img19/Pasted image 20250601121814.png" >

Por ejemplo, para los 3 servicios se puede ver que comparten el mismo `globalDeploymentId`

<img src="img19/Pasted image 20250601121943.png" >

<img src="img19/Pasted image 20250601122025.png" >

<img src="img19/Pasted image 20250601122045.png" >
...
- Mejora la composabilidad al separar la creación de datos globales, como `global_deployment_id` en este caso.
- Para la redundancia, el script `generate_global_metadata.py` se genera solo una vez por cada `terraform apply`, lo cual evitar redundancia
- En el caso de idempotencia, cada vez que ejecutaba `terraform apply` se obtenía un nuevo `globalDeploymentId`, por lo que no no se esta cumpliendo la idempotencia para este caso.
...
##### Paso 1
Modifica `initial_setup.sh` para que sea más idempotente: antes de crear `placeholder_...txt`, debe verificar si ya existe un archivo `placeholder_control.txt`. Si no existe, lo crea y también crea el `placeholder_...txt`. Si `placeholder_control.txt` ya existe, no hace nada más.


<img src="img19/Pasted image 20250601150731.png" >

Añadí los cambios para validar la creación solo si no existe el archivo:
...
# ...
echo "Readme se encuentra en: $README_PATH" >> setup_log.txt
if ls placeholder_*.txt 1>/dev/null 2>&1; then
    echo "Ya existe un archivo placeholder. No se creara uno nuevo"
else
    echo "Creando archivo de placeholder..."
    touch placeholder_$(date +%s).txt
echo "Setup inicial completado."
...
##### Paso 2
Ajusta los `triggers` del `null_resource "ejecutar_setup_inicial"` en el módulo `environment_setup` para que el script se ejecute de forma más predecible (quizás solo si una variable específica cambia).
...
resource "null_resource" "ejecutar_setup_inicial" {
  ...
  triggers = {
    readme_md5 = local_file.readme_entorno.content_md5 # Se reejecuta si el README cambia
    // Trigger añadido
    script_content_md5 = filemd5("${path.module}/scripts/initial_setup.sh")
  }
  provisioner "local-exec" {
	// ...
  }
...
<img src="img19/Pasted image 20250601154320.png" >


##### **Reto adicional:**
Implementa un "contador de ejecución" en un archivo dentro de `generated_environment`, que el script `initial_setup.sh` incremente solo si realmente realiza una acción.
...
##### Paso 1
Modifica el script `validate_config.py` para que busque explícitamente el contenido de `mensaje_global` (que el estudiante tendrá que "conocer" o pasar como argumento al script de validación) dentro de los archivos `config.json`. Si lo encuentra, debe marcarlo como un "hallazgo de seguridad crítico".

##### Paso 2
Discute cómo Terraform maneja los valores `sensitive` y cómo esto se puede perder si no se tiene cuidado al pasarlos a scripts o plantillas.


##### Paso 3
(Opcional) Modifica la plantilla `config.json.tpl` para ofuscar o no incluir directamente el `mensaje_global` si es demasiado sensible, tal vez solo una referencia.
