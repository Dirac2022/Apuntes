...
- El patrón Mediator es el que reduce el acoplamiento ya que elimina la comunicación directa entre componentes, sino que estos se comunican a través de un objeto o interfaz Mediator que redirecciona las llamadas a los componentes adecuados. En términos de reutilización, Adapter permite que los componentes sean reutilizables, ya que al existir un objeto Mediator, estas clases no están relacionadas entre si. 
- El patrón adapter permite reutilización de código ya que esta clase o interfaz Adapter puede ser modificable o extensible si por ejemplo estábamos trabajando con una API y hemos decidido cambiar de API o tecnología. Por lo tanto nuestra aplicación base no se modificaría, solo tendríamos que cambiar el Adaptador
